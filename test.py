...
print(size_lamex)
print(size_lamey)
crystallinity = 0.3 # percentage of unit in crystals region
...
# sites hold all unit on lattic in 4 dimention matrix
	...
	# generate "bridge" type amorphous
	...
	while st_count<num_bridge and bridge_count < int(amo_sites):
		...
		# print(atoms_list[l_rand*height_lame])
		# print(atoms_list[l_rand*height_lame:l_rand*height_lame+height_lame])
		if switch == 0:
	...
	for x in range(1, len(list_atoms_in_chains)):
		if type_arr == 2:
			list_atoms_in_chains[x]['type'] = list_atoms_in_chains[x-1]['type'] + 1
			type_arr = 1
		elif type_arr ==1:
			list_atoms_in_chains[x]['type'] = list_atoms_in_chains[x-1]['type']
			type_arr = 2
		if list_atoms_in_chains[x]['type'] > 2:
